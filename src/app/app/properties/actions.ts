...
const schema = z.object({
  ...
  postalCode: z.string().trim().regex(/^\d{5}$/, "Bitte fünfstellige PLZ eingeben"),
...
export async function createProperty(_: PropertyFormState, formData: FormData): Promise<PropertyFormState> {
  ...
  const unitQueries = Array.from({ length: parsed.data.unitCount }, (_, index) => {
    const label = parsed.data.unitCount === 1 ? "Einheit 1" : `Einheit ${index + 1}`;
  ...
  try {
    ...
    return { error: "Das Objekt konnte nicht gespeichert werden. Bitte versuchen Sie es erneut." };
  }
