...
import { CalendarClock, CheckCircle2, CircleDollarSign, Wrench } from "lucide-react";
import { requireSession } from "@/auth/session";
import { AppShell } from "@/components/app-shell";
import { Badge, PageHeader } from "@/components/ui";
import { getDb } from "@/db/client";
import { contacts, maintenanceCases } from "@/db/schema";
import { listOrganizationProperties, listOrganizationUnits } from "@/repositories/portfolio";
...
export default async function MaintenancePage({ searchParams }: { searchParams: Promise<{ status?: string; new?: string; error?: string; created?: string }> }) {
  ...
  const visible = query.status && query.status !== "all" ? items.filter((item) => item.status === query.status) : items;
  const labels = new Map([...properties.map((item) => [item.id, item.name] as const), ...units.map((item) => [item.id, `${item.propertyName} · ${item.label}`] as const)]);
  const names = new Map(contactRows.map((item) => [item.id, item.company || item.name]));
  const estimated = items.filter((item) => item.status !== "resolved").reduce((sum, item) => sum + (item.estimatedCostCents || 0), 0);
  const overdue = items.filter((item) => item.status !== "resolved" && item.dueAt && item.dueAt < new Date()).length;
  const activeFilter = query.status && query.status !== "all" ? query.status : "all";
    ...
    <AppShell active="/app/maintenance">
      ...
      <section className="kpi-grid">
        <article>
          ...
          <small>{items.filter((item) => item.priority === "urgent" && item.status !== "resolved").length} dringend</small>
        </article>
        <article>
      ...
      <details className="detail-panel create-drawer" id="maintenance-create" open={query.new === "1"}>
      ...
      <nav className="filter-tabs">
        <Link className={!query.status || query.status === "all" ? "active" : ""} href="/app/maintenance?status=all">
          Alle
        </Link>
        <Link className={query.status === "open" ? "active" : ""} href="/app/maintenance?status=open">
          Offen
        </Link>
        <Link className={query.status === "scheduled" ? "active" : ""} href="/app/maintenance?status=scheduled">
          Terminiert
        </Link>
        <Link className={query.status === "resolved" ? "active" : ""} href="/app/maintenance?status=resolved">
          Erledigt
        </Link>
      </nav>
      {visible.length ? (
        <div className="task-list">
          {visible.map((item, index) => (
            <Link href={`/app/maintenance/${item.id}`} className="task-item" key={item.id}>
              <span className="task-index">{String(index + 1).padStart(2, "0")}</span>
              <div className="task-copy">
                <span className="eyebrow">
                  {categoryLabels[item.category]} · {labels.get(item.unitId || item.propertyId || "") || "Allgemein"}
                </span>
                <h3>{item.title}</h3>
                <p>
                  {names.get(item.assigneeContactId || "") || "Noch nicht zugewiesen"}
                  {item.dueAt ? ` · fällig ${item.dueAt.toLocaleDateString("de-DE")}` : ""}
                </p>
              </div>
              <div className="task-meta">
                <Badge tone={item.status === "resolved" ? "success" : item.priority === "urgent" ? "urgent" : "warning"}>{item.status === "resolved" ? "Erledigt" : item.status === "scheduled" ? "Terminiert" : item.priority === "urgent" ? "Dringend" : "Offen"}</Badge>
                <strong>{item.actualCostCents != null ? money.format(item.actualCostCents / 100) : item.estimatedCostCents != null ? `~ ${money.format(item.estimatedCostCents / 100)}` : ""}</strong>
              </div>
            </Link>
          ))}
        </div>
        ...
        <section className="empty-state maintenance-empty" aria-live="polite">
          <span className="empty-icon">{activeFilter === "resolved" ? <CheckCircle2 size={24} /> : <Wrench size={24} />}</span>
          <span className="eyebrow">Aktueller Filter</span>
          <h2>Keine Vorgänge</h2>
          <p>{emptyCopy}</p>
          <Link className="btn btn-quiet" href={`/app/maintenance?status=${activeFilter}&new=1#maintenance-create`}>
