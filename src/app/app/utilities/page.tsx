...
import { ArrowRight, Building2, CalendarClock, CircleDollarSign, FileCheck2, Plus } from "lucide-react";
...
export default async function UtilitiesPage({ searchParams }: { searchParams: Promise<{ propertyId?: string; unitId?: string }> }) {
  const session = await requireSession();
  const query = await searchParams;
  const [periods, costs, properties, units] = await Promise.all([getDb().select().from(utilityPeriods).where(eq(utilityPeriods.organizationId, session.organizationId)).orderBy(desc(utilityPeriods.endsAt)), getDb().select().from(utilityCostItems).where(eq(utilityCostItems.organizationId, session.organizationId)), listOrganizationProperties(session.organizationId), listOrganizationUnits(session.organizationId)]);
  const selectedProperty = properties.find((property) => property.id === query.propertyId) ?? null;
  const selectedUnit = units.find((unit) => unit.id === query.unitId && unit.propertyId === selectedProperty?.id) ?? null;
  ...
  return <AppShell active="/app/utilities">
    ...
    {properties.length ? <div className="dashboard-grid"><form action={createUtilityPeriod} className="form-sheet compact-form"><div className="form-section-heading"><span><Plus size={16} /></span><div><h2>Neue Periode</h2><p>Objekt und Abrechnungszeitraum festlegen.</p></div></div>{selectedUnit && <p className="context-note">Gestartet für {selectedUnit.propertyName} · {selectedUnit.label}. Die Periode gilt für das gesamte Objekt; Einzelabrechnungen werden anschließend je Einheit und Mietverhältnis erzeugt.</p>}<div className="form-grid"><label className="field wide"><span>Objekt</span><select name="propertyId" required defaultValue={selectedProperty?.id}>{properties.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="field wide"><span>Titel</span><input name="title" required placeholder="Betriebskosten 2026" /></label><label className="field"><span>Von</span><input type="date" name="startsAt" required /></label><label className="field"><span>Bis</span><input type="date" name="endsAt" required /></label></div><div className="form-actions"><button className="btn">Periode anlegen</button></div></form>
      <section className="detail-panel"><div className="panel-title"><h2>Abrechnungsablauf</h2></div><ol className="workflow-list"><li><b>1</b><span><strong>Belege & Kosten</strong><small>Rechnungen hochladen, Daten prüfen, Kostenart wählen.</small></span></li><li><b>2</b><span><strong>Umlage & Nutzerwechsel</strong><small>Fläche, Einheiten, Verbrauch und Mietzeiträume berücksichtigen.</small></span></li><li><b>3</b><span><strong>Prüfen & abschließen</strong><small>Vorauszahlungen, Saldo und Belegeinsicht kontrollieren.</small></span></li></ol></section></div> : <section className="empty-state"><Building2 size={28} /><h2>Zuerst ein Objekt anlegen</h2><p>Betriebskostenperioden sind immer objektbezogen. Danach entstehen Einzelabrechnungen je Einheit und Mietverhältnis.</p><Link className="btn" href="/app/properties/new">Objekt anlegen</Link></section>}
