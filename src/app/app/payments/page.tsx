...
export default async function PaymentsPage({ searchParams }: { searchParams: Promise<{ tenancyId?: string }> }) {
  ...
  const selectedTenancyId = tenancyOptions.some((item) => item.id === query.tenancyId) ? query.tenancyId : undefined;
  ...
  return <AppShell active="/app/payments">
    ...
    <div className="finance-entry-grid">
      <details className="form-sheet compact-form create-drawer" open={Boolean(selectedTenancyId) || !rows.length}><summary className="form-section-heading"><span>+</span><div><h2>Zahlung manuell buchen</h2><p>Sollstellung oder bereits eingegangene Zahlung erfassen.</p></div></summary>{tenancyOptions.length ? <form action={createManualPayment} className="embedded-form"><div className="form-grid"><label className="field wide"><span>Mietverhältnis</span><select name="tenancyId" required defaultValue={selectedTenancyId || ""}><option value="">Bitte auswählen</option>{tenancyOptions.map((item) => <option value={item.id} key={item.id}>{item.property} · {item.unit} · {item.first} {item.last}</option>)}</select></label><label className="field"><span>Betrag €</span><input type="number" min="0.01" step="0.01" name="amount" required /></label><label className="field"><span>Fällig am</span><input type="date" name="dueAt" required /></label><label className="field"><span>Bezahlt am <small>optional</small></span><input type="date" name="paidAt" /></label><label className="field"><span>Verwendungszweck</span><input name="reference" placeholder="z. B. Miete 08/2026" /></label></div><div className="form-actions"><button className="btn">Buchung speichern</button></div></form> : <div className="relation-empty"><p>Für eine Buchung wird zuerst ein Mietverhältnis benötigt.</p><a className="btn secondary" href="/app/tenancies/new" target="_blank" rel="noreferrer">Mietverhältnis anlegen ↗</a></div>}</details>
