...
export default async function AnalyticsPage() {
  ...
  return <AppShell active="/app/analytics"><PageHeader eyebrow="Portfolio Intelligence" title="Analyse & Potenziale" description="Finanzen, Vermietung, Betriebskosten und Instandhaltung aus dem aktuellen Datenbestand." />
    ...
    <AnalyticsCharts monthly={data.monthly} properties={data.properties} utility={data.utilityByCategory} />
    <div className="section-heading dossier-section-title"><div><span className="eyebrow">Handlungspotenziale</span><h2>Finanzielle Hebel</h2></div></div><div className="dashboard-grid"><section className="potential"><PiggyBank size={21}/><span className="eyebrow">Leerstand</span><strong>{money.format(data.vacancyPotential)} / Monat</strong><p>Planmiete freier Einheiten</p></section><section className="potential"><PiggyBank size={21}/><span className="eyebrow">Zielmietendifferenz</span><strong>{money.format(data.rentGap)} / Monat</strong></section><section className="potential"><AlertTriangle size={21}/><span className="eyebrow">Betrieb & Wartung</span><strong>{money.format(data.utilityTotal + data.maintenanceCost)}</strong><p>Erfasste Gesamtkosten</p></section></div>
  </AppShell>;
}
