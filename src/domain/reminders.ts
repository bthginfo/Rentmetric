...
export type ReminderInput = {
  ...
  receivables: Array<{ id: string; label: string; dueAt: Date; openCents: number }>;
};

const legalNotice = "Automatisch abgeleiteter Hinweis – keine Rechtsberatung. Voraussetzungen und lokale Regelungen bitte prüfen.";
...
export function deriveReminders(input: ReminderInput): ReminderCandidate[] {
  ...
  for (const tenancy of input.tenancies) {
    if (tenancy.endsAt) {
      ...
      if (days <= 120) reminders.push({
        ...
        description: days < 0 ? `Das hinterlegte Vertragsende ist seit ${Math.abs(days)} Tagen überschritten.` : `Das Vertragsende ist in ${days} Tagen erreicht. Anschlussprozess und Fristen prüfen.`,
    ...
    if (tenancy.depositPaidCents < tenancy.depositDueCents) reminders.push({
      ...
      description: "Vereinbarter und verbuchter Kautionsbetrag weichen voneinander ab.",
      dueAt: today,
      severity: "attention",
      confidence: "verified",
      source: { type: "tenancy", id: tenancy.id },
    ...
    if (tenancy.lastRentIncreaseAt) {
      ...
      reminders.push({
        ...
        description: "Der zeitliche Prüfpunkt ist erreicht. Vergleichsmiete, Kappungsgrenze, Begründung und Sperrfristen separat validieren.",
        dueAt: reviewAt,
  ...
  for (const document of input.documents) {
    ...
    if (days <= 90) reminders.push({
      ...
      description: days < 0 ? `Seit ${Math.abs(days)} Tagen abgelaufen.` : `Noch ${days} Tage gültig. Erneuerung vorbereiten.`,
  ...
  for (const receivable of input.receivables) {
    if (receivable.openCents <= 0 || !isBefore(receivable.dueAt, today)) continue;
    reminders.push({
      ...
      description: "Offener Saldo nach Fälligkeit. Zahlungseingang und Zuordnung prüfen.",
