...
export function percentageOfCents(amountCents: number, basisPoints: number): number {
  ...
  if (!Number.isInteger(basisPoints)) throw new TypeError("basisPoints muss ganzzahlig sein");
...
function assertCents(value: number) {
  if (!Number.isSafeInteger(value)) throw new TypeError("Geldbeträge müssen als sichere Integer-Cent vorliegen");
}
